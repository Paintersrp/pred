...
function Calculator() {
  ...
  const [oddsNum, setOdds] = useState(null);
  const [wagerNum, setWager] = useState(null);
  const [finalNum, setFinal] = useState(null);
  const [decimalNum, setDecimal] = useState(null);
  const [americanNum, setAmerican] = useState(null);
  const [impliedOdds, setImplied] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    const data = await axios
      ...
      .then((res) => {
        ...
        setAmerican(res.data.american);
        setImplied(res.data.implied);
    ...
    <div className="layout flex-wrap">
      <div className="setwidth content-center border-r border-l">
        <div className="calc-select flex flex-row">
          <Link to="/calc/payout" className="btn-link">
            <button className="selected">Payout</button>
          </Link>
        ...
        <div
          className={
            toggleState === 1
              ? "content  active-content CalcTabs pb-10"
              : "content CalcTabs"
          }
        >
          <h2 className="mt-6 mb-10 title">Payout Calculator</h2>
          ...
          <form onSubmit={handleSubmit} className="form mb-12">
            <label className="">Betting Odds</label>
            <input
              type="number"
              step="0.01"
          ...
          <div className="div mb-10">
            <h3 className="align-center mb-2">Payout Results</h3>
            <hr className="mb-4" />
            ...
            <div className="flex flex-row align-center">
              ...
              <label className="flex-1 flex-row align-center border text-[14px]">
                {Math.round((finalNum - wagerNum + Number.EPSILON) * 100) / 100}
              </label>
            </div>
          </div>
          <div className="div">
            <h2 className="align-center mb-2">Converted Odds</h2>
            <hr className="mb-4" />
            <div className="flex flex-row align-center">
              <label className="flex-1 flex-row align-center text-[14px]">
                American
      ...
      <div className="setwidth-info border-r border-l flex flex-col flex-wrap info-sm">
        ...
        <div className="active-content flex">
          ...
          <p className="text-white p-2 text-[13px]">
            Net Profit is simply the payout total of your wager minus the value
            you wagered.
          </p>
        </div>
      </div>
